package util.launcher {
	...
	import util.assetLoading.AssetEvent;
	import util.assetLoading.AssetLoader;
	...
	public class LaunchManager {
		...
		private function errorReport(event : AssetEvent) : void {
			...
			this.dispatcher.dispatchEvent(new LauncherEvent(STEP_ERROR_EVENT,event.url));
		...
		private function onComplete(event : AssetEvent) : void {
			...
			this.dispatcher.dispatchEvent(new LauncherEvent(LaunchManager.INSIDE_EVENT));
			if (this._currentStep == this._step) {
				this.allComplete();
			}
		}
...
//				ExternalInterface.call("console.info", "[LaunchManager/allComplete] !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
				this.dispatcher.dispatchEvent(new LauncherEvent(LAUNCH_COMPLETE_EVENT,JSON.stringify(this._allReport)));
			}
		}
		...
		private function onInside(event:Event = null) : void {
			while (this._launcherBuffer.length > 0) {
				...
				this.dispatcher.dispatchEvent(new LauncherEvent(SETUP_EVENT,name));
			}
		}
